...
vim.opt.ruler = true           -- 状态栏显示光标位置（已注释，因状态栏已关闭）
vim.opt.laststatus = 3         -- 完全隐藏底部状态栏
vim.opt.termguicolors = true   -- 启用真彩色（需要终端支持）
vim.opt.showmode = true        -- 不显示模式提示（--INSERT--），因为状态栏已隐藏
...
vim.opt.linebreak = false      -- 不在单词中间换行（配合wrap使用）
...
vim.opt.foldlevel = 99         -- 默认展开所有折叠（99层）
vim.opt.foldtext = "v:lua.vim.fn.getline(v:foldstart) .. ' …'"  -- 折叠行显示文本：首行内容 + …
